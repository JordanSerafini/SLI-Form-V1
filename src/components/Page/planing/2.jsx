...
function PlaningPage() {
  ...
  const eventItems = filteredEvents.map((event) => {
    ...
    const flippedClass = isFlip[event.id] ? "flipped" : "";
    const cardClass = isFlip ? ".flip " : "";
    ...
    return (
      <div
        key={event.id}
        className={`overflow-hidden border-2 border-10c rounded-xl shrink-0 flex w-72 h-52 ${cardColorClass} ${cardClass}`}
      ...
      >
        {isFlip[event.id] ? (
          ...
          <div className={`w-8/10 flex flex-row items-center justify-center p-2 flip `}>
            {event.workingduration_editedduration && (
              <div>durée prévu: {event.workingduration_editedduration}</div>
            )}
            <div className="text-white">Note: {event.notesclear}</div>
